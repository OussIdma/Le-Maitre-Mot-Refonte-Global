...
async def sync_admin_to_exercise_types(
    ...
    query = {"is_dynamic": True, "generator_key": {"$exists": True, "$ne": None}}
    if chapter_code:
    ...
    # Grouper par (chapter_code, generator_key) pour éviter les doublons
    seen = set()
    ...
    async for ex in admin_exercises.find(query):
        ...
        chapter_normalized = chapter.upper().replace("-", "_") if chapter else None
        
        # Vérifier si déjà traité (idempotent)
        key = (chapter_normalized, generator_key)
        if key in seen:
            print(f"  ⏭️  Skipped (déjà traité): {chapter_normalized} / {generator_key}")
            skipped_count += 1
            continue
        
        seen.add(key)
        
        print(f"🔄 Traitement: {chapter_normalized}/{generator_key}")
        
        # Vérifier si exercise_type existe déjà
        existing = await exercise_types.find_one({
            "code_ref": generator_key,
            "chapter_code": chapter_normalized
        })
        ...
        try:
            ...
            niveau = chapter_normalized.split('_')[0] if chapter_normalized else "6E"
            
            # Créer le document exercise_type
            exercise_type_doc = {
                "id": f"{chapter_normalized}_{generator_key}_{uuid.uuid4().hex[:8]}",
                "code_ref": generator_key,
                "chapter_code": chapter_normalized,
                "chapitre_id": chapter_normalized,  # Fallback legacy
                "niveau": niveau,
                "domaine": _infer_domain(chapter_normalized),
                "titre": ex.get("title") or f"Exercice {generator_key}",
...
def _infer_domain(chapter_code):
    ...
    # Ex: "6E_GM07" → "GM" → "Grandeurs et Mesures"
