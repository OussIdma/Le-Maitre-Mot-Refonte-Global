...
PR2: DB ONLY - Plus de dépendance aux fichiers Python (data/gm08_exercises.py).
...
def _format_exercise_response(exercise: Dict[str, Any], timestamp: int) -> Dict[str, Any]:
    ...
    exercise_id = f"ex_6e_gm08_{exercise_id_db}_{timestamp}"
    ...
    return {
        ...
        "metadata": {
            ...
            "generator_code": f"6e_GM08_{exercise.get('family', 'NONE')}",
            "family": exercise.get("family"),
            "exercise_type": exercise.get("exercise_type"),
            "exercise_id": exercise_id_db,
            "is_fallback": False,
            "source": "gm08_db_exercises",
...
async def generate_gm08_exercise(
    ...
    """
    Génère UN exercice GM08 depuis MongoDB.
    ...
    pool = await repo.list_by_chapter("6E_GM08", offer=offer, difficulty=difficulty)
    
    if not pool:
        logger.warning(f"[GM08] Aucun exercice disponible pour offer={offer}, difficulty={difficulty}")
        return None
...
async def generate_gm08_batch(
    ...
    if pool_size == 0:
        ...
        logger.warning(f"[GM08] {batch_meta['warning']}")
        return [], batch_meta
...
async def get_gm08_available_exercises(
    db: AsyncIOMotorDatabase,
    offer: Optional[str] = None,
    difficulty: Optional[str] = None
) -> Dict[str, Any]:
    ...
    """
    repo = StaticExerciseRepository(db)
    exercises = await repo.list_by_chapter("6E_GM08", offer=offer, difficulty=difficulty)
