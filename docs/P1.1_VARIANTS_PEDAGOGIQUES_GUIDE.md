# P1.1 - VARIANTS PÉDAGOGIQUES (A/B/C) - GUIDE D'IMPLÉMENTATION

**Date**: 23 décembre 2025  
**Statut**: 🟡 EN COURS (paramètre ajouté, logique à implémenter)  
**Objectif**: Différenciation pédagogique sans changer le niveau mathématique

---

## 🎯 OBJECTIF

Permettre à un même générateur de produire 3 variants d'un même exercice:
- **A (Standard/Autonome)**: Énoncé direct, solution complète mais concise
- **B (Guidé)**: Énoncé avec indices/questions intermédiaires, solution très détaillée
- **C (Diagnostic)**: Énoncé propose une réponse à vérifier/justifier

**Contrainte**: Même difficulté = même niveau mathématique. Le variant change la **pédagogie**, pas le **calcul**.

---

## ✅ CE QUI EST DÉJÀ FAIT

### 1. Paramètre `variant_id` ajouté au schéma

**Fichiers modifiés**:
- `backend/generators/raisonnement_multiplicatif_v1.py`
- `backend/generators/calcul_nombres_v1.py`

```python
ParamSchema(
    name="variant_id",
    type=ParamType.ENUM,
    description="Variant pédagogique (P1.1)",
    default="A",
    options=["A", "B", "C"],
    required=False
)
```

**Impact**: Le paramètre est maintenant disponible dans `params` lors de la génération.

---

## 📋 CE QU'IL RESTE À FAIRE

### ÉTAPE 1 : Extraire `variant_id` dans la méthode `generate()`

**Fichier**: `backend/generators/raisonnement_multiplicatif_v1.py`  
**Méthode**: `generate(self, params: Dict[str, Any])`

```python
def generate(self, params: Dict[str, Any]) -> Dict[str, Any]:
    # Extraire les paramètres
    exercise_type = params.get("exercise_type", "proportionnalite_tableau")
    difficulty = params.get("difficulty", "moyen")
    grade = params.get("grade", "6e")
    seed = params.get("seed")
    variant_id = params.get("variant_id", "A")  # ← AJOUTER ICI
    
    # ... validation ...
    
    # Passer variant_id aux méthodes _generate_*
    if exercise_type == "proportionnalite_tableau":
        variables = self._generate_proportionnalite_tableau(difficulty, grade, variant_id)
    elif exercise_type == "pourcentage":
        variables = self._generate_pourcentage(difficulty, grade, variant_id)
    # ... etc
```

---

### ÉTAPE 2 : Créer des helpers pour adapter énoncé/solution

**Fichier**: `backend/generators/raisonnement_multiplicatif_v1.py`  
**Nouvelle méthode** (à ajouter après `_build_tableau_html`)

```python
def _adapt_enonce_variant(
    self, 
    base_enonce: str, 
    variant_id: str, 
    exercise_context: Dict[str, Any]
) -> str:
    """
    Adapte l'énoncé selon le variant pédagogique.
    
    Args:
        base_enonce: Énoncé de base (variant A)
        variant_id: "A", "B", ou "C"
        exercise_context: Contexte de l'exercice (données, valeurs, etc.)
    
    Returns:
        Énoncé adapté au variant
    """
    if variant_id == "A":
        # Standard: énoncé direct
        return base_enonce
    
    elif variant_id == "B":
        # Guidé: ajouter des indices/questions intermédiaires
        hints = []
        
        # Exemple pour proportionnalité
        if "coefficient" in exercise_context:
            hints.append("💡 Indice 1 : Commence par trouver le coefficient de proportionnalité.")
            hints.append("💡 Indice 2 : Divise une valeur de la ligne 2 par la valeur correspondante de la ligne 1.")
            hints.append("💡 Indice 3 : Multiplie la valeur de la ligne 1 par ce coefficient.")
        
        hints_text = "<br>".join(hints)
        return f"{base_enonce}<br><br><strong>Pour t'aider :</strong><br>{hints_text}"
    
    elif variant_id == "C":
        # Diagnostic: proposer une réponse à vérifier
        if "reponse_proposee" in exercise_context:
            reponse = exercise_context["reponse_proposee"]
            correct = exercise_context.get("reponse_correcte", True)
            
            return f"{base_enonce}<br><br><strong>Un élève propose :</strong> {reponse}<br><br>Cette réponse est-elle correcte ? Justifie ta réponse."
        else:
            # Fallback si pas de réponse proposée
            return base_enonce
    
    return base_enonce


def _adapt_solution_variant(
    self, 
    base_solution: str, 
    base_calculs: str,
    variant_id: str, 
    exercise_context: Dict[str, Any]
) -> tuple[str, str]:
    """
    Adapte la solution selon le variant pédagogique.
    
    Args:
        base_solution: Solution de base (variant A)
        base_calculs: Calculs intermédiaires de base
        variant_id: "A", "B", ou "C"
        exercise_context: Contexte de l'exercice
    
    Returns:
        (solution_adaptée, calculs_adaptés)
    """
    if variant_id == "A":
        # Standard: solution complète mais concise
        return base_solution, base_calculs
    
    elif variant_id == "B":
        # Guidé: solution très détaillée avec explications
        calculs_detailles = base_calculs.replace("Étape", "📝 Étape")
        calculs_detailles += "\n\n✅ Vérification : Tous les calculs sont corrects."
        
        solution_detaillee = base_solution + "\n\n💡 Explication : " + \
            "En proportionnalité, toutes les valeurs sont multipliées par le même coefficient. " + \
            "C'est pourquoi on divise d'abord pour trouver ce coefficient, puis on multiplie."
        
        return solution_detaillee, calculs_detailles
    
    elif variant_id == "C":
        # Diagnostic: justification de la réponse
        correct = exercise_context.get("reponse_correcte", True)
        
        if correct:
            solution_diagnostic = f"✅ La réponse proposée est CORRECTE.\n\n{base_solution}"
        else:
            solution_diagnostic = f"❌ La réponse proposée est INCORRECTE.\n\n{base_solution}\n\n" + \
                f"Erreur identifiée : {exercise_context.get('erreur_commise', 'Calcul erroné')}"
        
        return solution_diagnostic, base_calculs
    
    return base_solution, base_calculs
```

---

### ÉTAPE 3 : Adapter chaque méthode `_generate_*`

**Exemple pour `_generate_proportionnalite_tableau`**

#### Avant (actuel)
```python
def _generate_proportionnalite_tableau(self, difficulty: str, grade: str) -> Dict[str, Any]:
    # ... génération des valeurs ...
    
    enonce = f"{intro}<br><br>Quelle est la valeur manquante ?"
    
    return {
        "enonce": enonce,
        "consigne": self.rng_choice(self._CONSIGNE_VARIANTS["proportionnalite_tableau"]),
        "solution": solution,
        "calculs_intermediaires": calculs,
        "reponse_finale": str(valeur_a_trouver),
        # ...
    }
```

#### Après (avec variants)
```python
def _generate_proportionnalite_tableau(
    self, 
    difficulty: str, 
    grade: str, 
    variant_id: str = "A"
) -> Dict[str, Any]:
    # ... génération des valeurs (INCHANGÉ) ...
    
    # Énoncé de base (variant A)
    base_enonce = f"{intro}<br><br>Quelle est la valeur manquante ?"
    
    # Contexte pour les variants
    exercise_context = {
        "coefficient": coeff,
        "valeur_base": valeurs_base[index_a_trouver],
        "valeur_a_trouver": valeur_a_trouver,
        "reponse_proposee": None,  # À générer pour variant C
        "reponse_correcte": True,
    }
    
    # Variant C : générer une réponse proposée (correcte ou incorrecte)
    if variant_id == "C":
        if self._rng.random() < 0.5:
            # Proposer la bonne réponse
            exercise_context["reponse_proposee"] = str(valeur_a_trouver)
            exercise_context["reponse_correcte"] = True
        else:
            # Proposer une réponse erronée (erreur classique: addition au lieu de multiplication)
            reponse_erronee = valeurs_base[index_a_trouver] + coeff
            exercise_context["reponse_proposee"] = str(reponse_erronee)
            exercise_context["reponse_correcte"] = False
            exercise_context["erreur_commise"] = "Addition au lieu de multiplication"
    
    # Adapter l'énoncé selon le variant
    enonce = self._adapt_enonce_variant(base_enonce, variant_id, exercise_context)
    
    # Adapter la solution selon le variant
    solution_adaptee, calculs_adaptes = self._adapt_solution_variant(
        solution, calculs, variant_id, exercise_context
    )
    
    return {
        "enonce": enonce,
        "consigne": self.rng_choice(self._CONSIGNE_VARIANTS["proportionnalite_tableau"]),
        "solution": solution_adaptee,
        "calculs_intermediaires": calculs_adaptes,
        "reponse_finale": str(valeur_a_trouver),
        "donnees": donnees,
        "tableau_html": tableau_html,
        "methode": "coefficient_de_proportionnalite",
        "niveau": grade,
        "type_exercice": "proportionnalite_tableau",
        "variant_id": variant_id,  # ← AJOUTER pour traçabilité
    }
```

---

### ÉTAPE 4 : Répéter pour les autres types d'exercices

Appliquer la même logique à:
- `_generate_pourcentage`
- `_generate_vitesse`
- `_generate_echelle`
- Tous les types de `CALCUL_NOMBRES_V1`

---

## 🧪 TESTS À CRÉER

**Fichier**: `backend/tests/test_variants_pedagogiques.py`

```python
"""Tests pour les variants pédagogiques A/B/C"""
import pytest
from backend.generators.raisonnement_multiplicatif_v1 import RaisonnementMultiplicatifV1Generator

class TestVariantsPedagogiques:
    """Tests des variants A/B/C."""
    
    def test_variant_a_standard(self):
        """Variant A : énoncé direct, solution concise."""
        gen = RaisonnementMultiplicatifV1Generator(seed=42)
        result = gen.generate({
            "exercise_type": "proportionnalite_tableau",
            "difficulty": "moyen",
            "grade": "6e",
            "variant_id": "A",
            "seed": 42
        })
        
        variables = result["variables"]
        assert variables["variant_id"] == "A"
        assert "💡 Indice" not in variables["enonce"]  # Pas d'indices en A
        assert "Un élève propose" not in variables["enonce"]  # Pas de diagnostic en A
    
    def test_variant_b_guide(self):
        """Variant B : énoncé avec indices, solution détaillée."""
        gen = RaisonnementMultiplicatifV1Generator(seed=42)
        result = gen.generate({
            "exercise_type": "proportionnalite_tableau",
            "difficulty": "moyen",
            "grade": "6e",
            "variant_id": "B",
            "seed": 42
        })
        
        variables = result["variables"]
        assert variables["variant_id"] == "B"
        assert "💡 Indice" in variables["enonce"]  # Indices présents
        # Solution plus détaillée
        assert len(variables["solution"]) > len(self.test_variant_a_standard().variables["solution"])
    
    def test_variant_c_diagnostic(self):
        """Variant C : énoncé avec réponse proposée à vérifier."""
        gen = RaisonnementMultiplicatifV1Generator(seed=42)
        result = gen.generate({
            "exercise_type": "proportionnalite_tableau",
            "difficulty": "moyen",
            "grade": "6e",
            "variant_id": "C",
            "seed": 42
        })
        
        variables = result["variables"]
        assert variables["variant_id"] == "C"
        assert "Un élève propose" in variables["enonce"]  # Diagnostic présent
        assert ("✅" in variables["solution"] or "❌" in variables["solution"])  # Verdict
    
    def test_determinisme_variants(self):
        """Même seed → même variant généré."""
        gen1 = RaisonnementMultiplicatifV1Generator(seed=42)
        gen2 = RaisonnementMultiplicatifV1Generator(seed=42)
        
        result1 = gen1.generate({
            "exercise_type": "proportionnalite_tableau",
            "difficulty": "moyen",
            "grade": "6e",
            "variant_id": "B",
            "seed": 42
        })
        
        result2 = gen2.generate({
            "exercise_type": "proportionnalite_tableau",
            "difficulty": "moyen",
            "grade": "6e",
            "variant_id": "B",
            "seed": 42
        })
        
        assert result1["variables"]["enonce"] == result2["variables"]["enonce"]
```

---

## 📊 ESTIMATIONS

| Tâche | Complexité | Temps estimé |
|-------|-----------|--------------|
| Helpers `_adapt_*` | Moyenne | 30min |
| Adapter `_generate_proportionnalite_tableau` | Faible | 20min |
| Adapter `_generate_pourcentage` | Faible | 20min |
| Adapter `_generate_vitesse` | Faible | 20min |
| Adapter `_generate_echelle` | Faible | 20min |
| Adapter `CALCUL_NOMBRES_V1` (3 types) | Moyenne | 1h |
| Tests complets | Moyenne | 45min |
| **TOTAL** | | **~3h** |

---

## 🎯 PRIORITÉS

### P0 (Critique)
- ✅ Ajouter `variant_id` au schéma
- Implémenter UN type d'exercice en exemple (proportionnalite_tableau)
- Tests de base pour valider le concept

### P1 (Important)
- Étendre aux autres types d'exercices
- Tests complets pour tous les variants

### P2 (Nice to have)
- Interface admin pour prévisualiser les variants
- Statistiques d'utilisation des variants

---

## 🚀 COMMENCER MAINTENANT

Pour implémenter rapidement:

1. **Copier les helpers** `_adapt_enonce_variant` et `_adapt_solution_variant` dans `raisonnement_multiplicatif_v1.py`
2. **Modifier UNE méthode** `_generate_proportionnalite_tableau` selon l'exemple
3. **Tester** avec un appel API:
   ```bash
   curl -X POST http://localhost:8000/api/v1/exercises/generate \
     -d '{"code_officiel": "6e_SP03", "offer": "pro", "seed": 42, "variant_id": "B"}'
   ```
4. **Valider** que l'énoncé contient les indices

---

## 📝 NOTES

- Les variants ne changent PAS les valeurs numériques (déterminisme préservé)
- Le variant C nécessite de générer une "réponse proposée" (correcte ou fausse)
- Les templates HTML frontend (`ChapterExercisesAdminPage.js`) n'ont PAS besoin de changer
- Le `variant_id` est optionnel (default "A"), donc pas de breaking change

---

**Statut global P1.1**: 🟡 **30% COMPLÉTÉ**  
- ✅ Paramètre ajouté
- 🔄 Logique à implémenter
- ⏳ Tests à créer




