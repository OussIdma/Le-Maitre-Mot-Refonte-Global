# P1.5 - API Admin Exercices Statiques ✅

**Date :** 2025-12-23  
**Statut :** ✅ **COMPLÉTÉ (Partie 2/3)**

---

## 🎯 Objectif

Rendre les **exercices statiques** (figés) visibles et éditables depuis l'admin, sans impacter les exercices dynamiques.

### Définition

**Exercice statique** = Exercice stocké en DB avec contenu `enonce_html`/`solution_html` déjà écrits.

**Caractéristiques :**
- `is_dynamic = False` (ou absent)
- `generator_key` absent ou `null`
- Contenu HTML figé (pas de variables/placeholders)

---

## 📦 Endpoints Créés

### Base URL
```
/api/v1/admin
```

---

### 1. **GET** `/chapters/{code_officiel}/static-exercises`

Liste tous les exercices statiques d'un chapitre.

#### Paramètres
- **`code_officiel`** (path) : Code du chapitre (ex: `6e_GM07`)

#### Réponse 200
```json
[
  {
    "id": 1,
    "chapter_code": "6e_GM07",
    "title": "Lecture de l'heure - 8h30",
    "difficulty": "facile",
    "enonce_html": "<p>Quelle heure indique l'horloge ?</p>",
    "solution_html": "<p><strong>Réponse :</strong> 8h30</p>",
    "tags": ["horloge", "lecture"],
    "order": 1,
    "exercise_type": "LECTURE_HEURE",
    "offer": "free",
    "updated_at": "2025-12-23T10:30:00Z"
  }
]
```

#### Filtrage
- **Inclus :** `is_dynamic=False` ET `generator_key` absent/null
- **Exclus :** Exercices dynamiques
- **Tri :** Par `order` (croissant), puis `id`

#### Erreurs
- **500** : Erreur serveur

---

### 2. **PUT** `/static-exercises/{exercise_id}`

Met à jour un exercice statique.

#### Paramètres
- **`exercise_id`** (path) : ID de l'exercice

#### Body (tous les champs optionnels)
```json
{
  "title": "Nouveau titre",
  "difficulty": "moyen",
  "enonce_html": "<p>Énoncé modifié...</p>",
  "solution_html": "<p>Solution modifiée...</p>",
  "tags": ["tag1", "tag2"],
  "order": 5,
  "exercise_type": "TYPE",
  "offer": "free"
}
```

#### Validation
- **`difficulty`** : `"facile"` | `"moyen"` | `"difficile"`
- **`offer`** : `"free"` | `"pro"`
- **`enonce_html`** : Max 10 000 caractères
- **`solution_html`** : Max 10 000 caractères
- **`title`** : Max 200 caractères

#### Réponse 200
```json
{
  "id": 1,
  "chapter_code": "6e_GM07",
  "title": "Nouveau titre",
  "difficulty": "moyen",
  ...
}
```

#### Erreurs
- **400** : Tentative de modification d'un exercice dynamique
  ```json
  {
    "detail": {
      "error_code": "CANNOT_UPDATE_DYNAMIC_VIA_STATIC_ENDPOINT",
      "message": "L'exercice 2 est dynamique. Utilisez l'endpoint dédié.",
      "is_dynamic": true,
      "generator_key": "LECTURE_HORLOGE",
      "hint": "Utilisez /api/v1/admin/exercises/{id} pour modifier les exercices dynamiques"
    }
  }
  ```
- **404** : Exercice introuvable
- **422** : Données invalides

---

### 3. **POST** `/chapters/{code_officiel}/static-exercises` ✅

Crée un nouvel exercice statique dans un chapitre.

#### Paramètres
- **`code_officiel`** (path) : Code du chapitre (ex: `6e_GM07`)

#### Body (tous les champs optionnels sauf enonce/solution)
```json
{
  "title": "Nouveau problème de durées",
  "difficulty": "moyen",
  "enonce_html": "<p>Énoncé du problème...</p>",
  "solution_html": "<p>Solution du problème...</p>",
  "tags": ["durees", "probleme"],
  "order": 10,
  "exercise_type": "PROBLEME_DUREES",
  "offer": "free"
}
```

#### Valeurs par défaut
- `difficulty`: `"facile"`
- `enonce_html`: `"<p>Énoncé à compléter...</p>"`
- `solution_html`: `"<p>Solution à compléter...</p>"`
- `offer`: `"free"`
- `tags`: `[]`
- `is_dynamic`: `False` (forcé)

#### Réponse 201
```json
{
  "id": 4,
  "chapter_code": "6e_GM08",
  "title": "Nouveau problème de durées",
  "difficulty": "moyen",
  ...
}
```

#### Erreurs
- **422** : Données invalides

---

### 4. **DELETE** `/static-exercises/{exercise_id}` ✅

Supprime un exercice statique.

#### Paramètres
- **`exercise_id`** (path) : ID de l'exercice

#### Réponse 204
Pas de contenu (succès)

#### Erreurs
- **400** : Tentative de suppression d'un exercice dynamique
- **404** : Exercice introuvable

---

## 🔒 Sécurité

### Protection des Exercices Dynamiques

Les endpoints **refusent strictement** toute opération (UPDATE, DELETE) sur un exercice dynamique :

```json
{
  "error_code": "CANNOT_UPDATE_DYNAMIC_VIA_STATIC_ENDPOINT",
  "message": "L'exercice X est dynamique. Utilisez l'endpoint dédié.",
  "is_dynamic": true,
  "generator_key": "GENERATOR_KEY"
}
```

**Rationnel :** 
- Éviter la corruption de données
- Séparer clairement les workflows statiques vs dynamiques
- Prévenir les erreurs utilisateur

---

## 🧪 Tests

### Fichier
`backend/tests/test_admin_static_exercises.py`

### Coverage

| Test | Description | Statut |
|------|-------------|--------|
| `test_list_static_by_chapter` | Liste exercices statiques | ✅ |
| `test_list_static_empty_chapter` | Chapitre vide → liste vide | ✅ |
| `test_update_static_success` | Mise à jour réussie | ✅ |
| `test_update_static_not_found` | Exercice inexistant → 404 | ✅ |
| `test_cannot_update_dynamic_via_static_endpoint` | ⚠️ **CRITIQUE** : Refus modification dynamique | ✅ |
| `test_update_static_invalid_difficulty` | Validation difficulté → 422 | ✅ |
| `test_update_static_invalid_offer` | Validation offre → 422 | ✅ |
| `test_create_static_success` | Création réussie → 201 | ✅ |
| `test_create_static_minimal` | Création avec valeurs par défaut | ✅ |
| `test_create_static_invalid_difficulty` | Validation création → 422 | ✅ |
| `test_delete_static_success` | Suppression réussie → 204 | ✅ |
| `test_delete_static_not_found` | Exercice inexistant → 404 | ✅ |
| `test_cannot_delete_dynamic_via_static_endpoint` | ⚠️ **CRITIQUE** : Refus suppression dynamique | ✅ |
| `test_static_exercises_preserved_after_operations` | Non-régression | ✅ |

**Total :** 14 tests | **Statut :** ✅ Tous PASS

---

## 📊 Architecture

### Modèle de Données (Réutilisé)

**Collection MongoDB :** `admin_exercises`

**Schéma (extraits pertinents) :**

```python
{
  "id": int,                      # ID unique
  "chapter_code": str,            # Code chapitre (ex: 6e_GM07)
  "title": Optional[str],         # Titre
  "difficulty": str,              # facile | moyen | difficile
  "enonce_html": str,             # Énoncé HTML (requis si statique)
  "solution_html": str,           # Solution HTML (requis si statique)
  "tags": List[str],              # Tags optionnels
  "order": Optional[int],         # Ordre d'affichage
  "exercise_type": Optional[str], # Type d'exercice
  "offer": str,                   # free | pro
  "is_dynamic": bool,             # False pour statiques
  "generator_key": Optional[str], # null pour statiques
  "created_at": datetime,
  "updated_at": datetime
}
```

### Service Réutilisé

**Service :** `ExercisePersistenceService`  
**Fichier :** `backend/services/exercise_persistence_service.py`

**Méthodes utilisées :**
- `get_all_by_chapter(chapter_code)` : Liste exercices d'un chapitre
- `get_by_id(exercise_id)` : Récupère un exercice
- `update(exercise_id, data)` : Met à jour un exercice
- `create(exercise_dict)` : Crée un exercice
- `delete(exercise_id)` : Supprime un exercice

---

## 🔄 Filtrage Statiques vs Dynamiques

### Logique de Filtrage

```python
# Un exercice est STATIQUE si:
is_static = (
    not exercise.get("is_dynamic", False) 
    and not exercise.get("generator_key")
)

# Un exercice est DYNAMIQUE si:
is_dynamic = (
    exercise.get("is_dynamic") == True 
    or exercise.get("generator_key") is not None
)
```

### Tri des Résultats

```python
# Tri par order (croissant), puis par id
exercises.sort(key=lambda ex: (ex.get("order") or 999999, ex.get("id", 0)))
```

**Rationnel :**
- `order` permet un tri personnalisé par l'admin
- `999999` : Si `order` absent → placé en fin de liste
- `id` : Ordre stable pour exercices sans `order`

---

## 🚀 Intégration

### Dans `server.py`

```python
# Routes admin exercices statiques (P1.5 - Partie 2/3)
from backend.routes.admin_static_exercises_routes import router as admin_static_exercises_router
app.include_router(admin_static_exercises_router, tags=["Admin Static Exercises"])
```

### Documentation Swagger

**URL :** http://localhost:8000/docs

**Tag :** `Admin Static Exercises`

---

## 📝 Exemples d'Utilisation

### Lister les exercices statiques d'un chapitre

```bash
curl -X GET "http://localhost:8000/api/v1/admin/chapters/6e_GM07/static-exercises"
```

### Modifier un exercice statique

```bash
curl -X PUT "http://localhost:8000/api/v1/admin/static-exercises/1" \
  -H "Content-Type: application/json" \
  -d '{
    "enonce_html": "<p>Énoncé modifié</p>",
    "solution_html": "<p>Solution modifiée</p>",
    "tags": ["nouveau", "tag"]
  }'
```

### Créer un nouvel exercice statique

```bash
curl -X POST "http://localhost:8000/api/v1/admin/chapters/6e_GM08/static-exercises" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Nouveau problème",
    "difficulty": "moyen",
    "enonce_html": "<p>Énoncé...</p>",
    "solution_html": "<p>Solution...</p>"
  }'
```

### Supprimer un exercice statique

```bash
curl -X DELETE "http://localhost:8000/api/v1/admin/static-exercises/1"
```

---

## ⚠️ Contraintes Respectées

- ✅ **Modèle existant réutilisé** : Aucune modification du schéma MongoDB
- ✅ **Endpoints publics intacts** : Aucune régression
- ✅ **Séparation statiques/dynamiques** : Filtrage strict via `is_dynamic`
- ✅ **Erreurs propres** : 404, 422, 400 avec `error_code` + `message` + `hint`
- ✅ **Tests obligatoires** : 14 tests passants

---

## 🎉 Livrables

| Livrable | Fichier | Statut |
|----------|---------|--------|
| Routes backend | `backend/routes/admin_static_exercises_routes.py` | ✅ |
| Tests | `backend/tests/test_admin_static_exercises.py` | ✅ |
| Documentation | `docs/P1.5_STATIC_EXERCISES_API.md` | ✅ |
| Intégration | `backend/server.py` | ✅ |

---

## 🚀 Prochaines Étapes (P1.5 - Partie 3/3)

### UI Admin : Onglets Statiques/Dynamiques

**Objectif :** Créer une interface admin avec 2 onglets distincts :

1. **"Dynamiques (générés)"** 
   - Liste des générateurs
   - Bouton "Rédaction templates"
   - Badge 🧩 Dynamique

2. **"Statiques (figés)"**
   - Liste des exercices statiques (consomme les endpoints créés ici)
   - Édition en ligne : `enonce_html`, `solution_html`, `tags`, `order`
   - Badge 📄 Statique

**Fichiers concernés :**
- `frontend/src/components/admin/ChapterExercisesAdminPage.js`

**Endpoints consommés :**
- `GET /api/v1/admin/chapters/{code}/static-exercises`
- `PUT /api/v1/admin/static-exercises/{id}`
- `POST /api/v1/admin/chapters/{code}/static-exercises`
- `DELETE /api/v1/admin/static-exercises/{id}`

---

**Date de livraison :** 2025-12-23  
**Temps passé :** ~2h  
**Statut Final :** ✅ **PARTIE 2/3 COMPLÈTE**








